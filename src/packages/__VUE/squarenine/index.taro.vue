...
export default create({
  ...
  setup(props, { emit, slots }) {
    ...
    const shuffle = (pos: number) => {
      ...
      query.selectAll(".nine-ninegrid__item").boundingClientRect();
      query.exec((res) => {
        const itemPos = res[0];
        itemPos.forEach((item: any, index: number) => {
          setTimeout(() => {
            css3Translate.value[index] = {
              x: state.arrPos[index].x + "px",
              y: state.arrPos[index].y + "px",
            };
          }, 150 * index);
        });
      });
    ...
    const calPosition = computed(() => (index: number) => {
      const sty: any = css3Translate.value[index];
      return sty ? { transform: `translate(${sty.x}, ${sty.y})` } : "";
    });
    /**
     * 返回游戏
     */
